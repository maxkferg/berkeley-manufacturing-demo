classdef epm2
    ...
    methods
        ...
        function obj = epm2(f_previous, newDataFileName, option)
...
%Figure out the cuttig direction
for i=1:length(energy)
...
%label depending on type of block
for i=1:length(energy)
...
%Define IDs based on these labels (basically group labels 3, 4 and 5)
for i=1:length(energy)
    % cut with feed
...
E = energy;
for i=1:length(E)
    if ID(i)==5 || ID(i)==6 || X(i,11)==0 %dwell
        L(i,1)=X(i,13);
    else
        L(i,1)=X(i,11);
    end
...
%output = {f1, f2, f3, f4, f5, f6, f7}, the GPML object
...
for i=1:length(E_test)
    if ID(i)==5 || ID(i)==6 || X_test(i,11)==0
        L_test(i,1)=X_test(i,13);
    else
...
%clean up data for cases where power meter has dropped
clean_up_index = setdiff([1:length(energy)],[index_zero]);
...
%clean up weird data - only for safety, should not ever have to do this!
