...
%Input : Xnew, Xprev, Ynew, Yprev, r, elements, block
...
% If angle is negative that indicates a G02 cut -> C = C(1,:)
    if isempty(strfind(blk,'G02')) == 0 || isempty(strfind(blk,'G2')) == 0; %Clockwise circle
    X = C(1,1); Y = C(1,2); tnew = C(1,4); tprev = C(1,5);
    elseif isempty(strfind(blk,'G03')) == 0 || isempty(strfind(blk,'G3')) == 0;%Counterclockwise circle
