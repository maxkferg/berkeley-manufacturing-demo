function [E_predict, S_predict ] = predictenergy2(epm, data)
...
%% Define Output Feature Vector

ID_test = input(:,12);
E_test = energy;
for i=1:size(E_test,1)
    if ID(i)==5 || ID(i)==6 || X_test(i,11)==0 %dwell
...
%clean up data
clean_up_index = setdiff(1:size(energy,1),index_zero);
