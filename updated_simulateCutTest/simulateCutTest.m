...
%             end
            ...
            for i=1:row %row
                for j=1:column %column
                    ...
                    if (seenFirstCut == false)
                        ...
                        firstCutDepth = val;
                        seenFirstCut = true;
                    elseif (val ~= firstCutDepth && j == column)
                        %case where last square in the x-y plane is a
                        %different cut
                        
                        %coords for the 1st block (same as the other cases)
                        x_n = (firstCut - 1);
                        x_o = (j-1);
                        y_n = row - (i - 1);
                        y_o = row - (i);
                        ...
                        z_o = 0;
                        vert = [vert; x_n y_o z_o; x_o y_o z_o; x_o y_n z_o; x_n y_o z_n; x_o y_o z_n; x_o y_n z_n; x_n y_n z_n];
                        ind=size(vert,1)/7;
                        ind2=7*(ind-1);
                        fac = [fac; 1+ind2 2+ind2 5+ind2 4+ind2; 4+ind2 7+ind2 6+ind2 5+ind2; 5+ind2 2+ind2 3+ind2 6+ind2];
                        
                        %coords for the last square, coords are in similar
                        %to the case in which j == column
                        x_n = j - 1;
                        x_o = j;
                        y_n = row - (i - 1);
                        y_o = row - (i);
                        z_n = height/ms + partmap(i,j)/ms;                         
                        z_o = 0;
                        vert = [vert; x_n y_o z_o; x_o y_o z_o; x_o y_n z_o; x_n y_o z_n; x_o y_o z_n; x_o y_n z_n; x_n y_n z_n];
                        ind=size(vert,1)/7;
                        ind2=7*(ind-1);
            ...
            patch('Vertices',vert,'Faces',fac,'FaceVertexCData',[0.2 0.2 0.8],'FaceColor','flat','EdgeColor',[.8 .2 .2])
            view(30,20)
            axis equal vis3d
            
%             patch(array(:,1), array(:,2), array(:,3), 0.2 * ones((size(array,1)),1));
%             view(3)
%             shading flat;
%             axis equal
...
%             title(['NC Block:  ' blk ', Depth :  ' num2str(Depth) ' mm , Feed :  ' num2str(feedrate) ' mm/min , Speed:  ' num2str(spindlespeed) ' RPM'])
            figure(2)
            subplot (2,2,1) %Second subplot : X,Y,Z,S Load bars
            hold off
            plot(NaN)
            H = [XL; YL; ZL; SL];
            ...
            subplot (2,2,2) %Third subplot : Energy graph
            energydata=EData(:,9);
